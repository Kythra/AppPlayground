...
public class board {
	...
	private boolean checkForWinner(int x, int y, int z, Stone color) {
		...
		// check level z
			...
			if (tmp > 3) {
				System.err.println("Wins in diagonal on level z = " + z);
				return true;
			}
			...
			if (tmp > 3) {
				System.err.println("Wins in x-axis on level z = " + z);
				return true;
			}
			// check y-axis
			i = 1;
			...
			if (tmp > 3) {
				System.err.println("Wins in y-axis on level z = " + z);
				return true;
			}
		// winner in z
		tmp =1;
		if (z>2){
			if (z>3){
				while(inRange(tmp) && positionOccupiedByPlayer(x, y, tmp, color)){
					tmp++;
				}
				if (tmp>3) {
					System.err.println("Wins in height 2 to 5 at position:" + x + ", " + y );
					return true;
				}
			}	
			tmp = 0;
			...
			if (tmp>3) {
				System.err.println("Wins in height 1 to 4 at position:" + x + ", " + y );
				return true;
			}
		}
		return false;
